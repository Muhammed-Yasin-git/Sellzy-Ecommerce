...
exports.adminOrderDetails = (req, res) => {
  ...
  orderdb.find({ _id: orderId }).then((data) => {
    console.log(data);
    ...
    .then((data) => {
      ...
      fs.unlink(imagepath, (err) => {
        ...
        productDb
          ...
          .then((deleteData) => {
            res.send(
              "<script>alert('Product deleted successfully!'); window.location='/admin-products';</script>"
            );
...
exports.userdetails = (req, res) => {
  ...
  Userdb.findOne({ _id: id }).then((response) => {
