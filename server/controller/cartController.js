            ...
            cartDb.find({ email: email })
              .then(cartData => {
                ...
                categorydb.find({ active: true })
                  .then(catData => {
                    const productData = cartData.map(item => item.cartQuantity);
              ...
              .catch(err => {
                console.error('Error finding cart data (outside the loop):', err);
                res.status(500).send(err);
              });
  ...
  productdb.findOne({ _id: id })
    .then(data => {
      ...
      cartDb.findOne({ prId: id, email: req.session.email })
        .then(data1 => {
          if (data1 && data1.cartQuantity < stock && data1.cartQuantity < 15) {
